(() => {
  ...
  function rotate(p){let x=p.x,y=p.y,z=p.z;const cy=Math.cos(yaw),sy=Math.sin(yaw);[x,z]=[x*cy-z*sy,x*sy+z*cy];const cp=Math.cos(pitch),sp=Math.sin(pitch);[y,z]=[y*cp-z*sp,y*sp+z*cp];return{x,y,z}}
  function project(p){const q=rotate(p),depth=4.2-q.z,scale=Math.min(w,h)*.2*zoom/depth;return{x:w/2+q.x*scale,y:h/2+q.y*scale,z:q.z,scale}}
  function makeSpace(){for(let i=0;i<1750;i++){const arm=i%4,a=arm*Math.PI/2+rand(-.22,.22),r=Math.pow(Math.random(),.64)*11.5;stars.push({x:Math.cos(a+r*.48)*r,y:rand(-.24,.24)*(1-r/14),z:Math.sin(a+r*.48)*r,s:rand(.55,1.95),a:rand(.3,1),c:Math.random()>.28?cfg.accent:'#fff'})}for(let i=0;i<360;i++){const a=rand(0,Math.PI*2),u=rand(-1,1),r=rand(11,20);dust.push({x:Math.cos(a)*Math.sqrt(1-u*u)*r,y:u*r,z:Math.sin(a)*Math.sqrt(1-u*u)*r,s:rand(.3,1.1)})}const layout=[[-7.4,-1.5,-1.4],[-5,-2.7,1.2],[0,-3.1,-1.2],[5,-2.7,1.4],[7.4,1.2,-1.4],[4.8,2.7,1.2],[0,3.1,-1.2],[-5,2.7,1.2]];cfg.messages.forEach((m,i)=>{const p=layout[i%layout.length];objects.push({x:p[0],y:p[1],z:p[2],message:m,kind:i%6,phase:rand(0,6.28)})})}
  function petal(x,y,rx,ry,rot,color){ctx.save();ctx.translate(x,y);ctx.rotate(rot);ctx.beginPath();ctx.ellipse(0,-ry*.45,rx,ry,0,0,Math.PI*2);ctx.fillStyle=color;ctx.fill();ctx.restore()}
  function rose(x,y,r,rot,alpha,kind){ctx.save();ctx.globalAlpha=alpha;ctx.translate(x,y);ctx.rotate(rot);ctx.shadowColor=cfg.glow;ctx.shadowBlur=r*.78;const stem=()=>{ctx.strokeStyle='#6fa45f';ctx.lineWidth=Math.max(1,r*.09);ctx.beginPath();ctx.moveTo(0,r*.45);ctx.quadraticCurveTo(r*.2,r,r*.02,r*1.65);ctx.stroke()};const center=(cx=0,cy=0,rr=.2)=>{ctx.beginPath();ctx.arc(cx,cy,r*rr,0,7);ctx.fillStyle=cfg.center;ctx.fill()};
    if(kind===0){for(let layer=0;layer<4;layer++)for(let i=0;i<7+layer*2;i++)petal(0,0,r*(.13+layer*.035),r*(.28+layer*.085),i*Math.PI/(3.5+layer)+layer*.28,layer>1?cfg.flower2:cfg.flower);center(0,0,.17)}
    else if(kind===1){for(let i=0;i<17;i++)petal(0,0,r*.15,r*.72,i*Math.PI/8.5,i%2?cfg.flower:cfg.flower2);center(0,0,.34)}
    else if(kind===2){stem();for(let j=0;j<3;j++){const ox=(j-1)*r*.64,oy=(j%2-.5)*r*.36;for(let i=0;i<9;i++)petal(ox,oy,r*.16,r*.54,i*Math.PI/4.5,cfg.flower2);center(ox,oy,.16)}}
    else if(kind===3){stem();for(let j=-1;j<=1;j++){const ox=j*r*.48,oy=Math.abs(j)*r*.18;ctx.save();ctx.translate(ox,oy);ctx.rotate(j*.18);ctx.beginPath();ctx.moveTo(-r*.34,0);ctx.quadraticCurveTo(-r*.26,-r*.75,0,-r*.82);ctx.quadraticCurveTo(r*.26,-r*.75,r*.34,0);ctx.quadraticCurveTo(0,r*.25,-r*.34,0);ctx.fillStyle=j?cfg.flower2:cfg.flower;ctx.fill();ctx.restore()}}
    else if(kind===4){stem();for(let j=0;j<6;j++){const a=j*Math.PI/3,ox=Math.cos(a)*r*.42,oy=Math.sin(a)*r*.32;for(let i=0;i<6;i++)petal(ox,oy,r*.11,r*.36,i*Math.PI/3,cfg.flower2);center(ox,oy,.12)}}
    else{ctx.strokeStyle='#78a968';ctx.lineWidth=Math.max(1,r*.07);for(let j=0;j<7;j++){const a=-1.05+j*.35,ox=Math.sin(a)*r*.75,oy=Math.cos(a)*r*.48;ctx.beginPath();ctx.moveTo(0,r*1.5);ctx.lineTo(ox,oy);ctx.stroke();for(let i=0;i<8;i++)petal(ox,oy,r*.13,r*.42,i*Math.PI/4,j%2?cfg.flower2:cfg.flower);center(ox,oy,.13)}ctx.fillStyle='rgba(255,244,184,.9)';ctx.beginPath();ctx.moveTo(-r*.72,r*.72);ctx.lineTo(0,r*1.58);ctx.lineTo(r*.72,r*.72);ctx.quadraticCurveTo(0,r*1.18,-r*.72,r*.72);ctx.fill()}
    ctx.restore()}
  function drawHeart(x,y,r,label,t){ctx.save();ctx.translate(x,y);ctx.shadowColor=cfg.glow;ctx.shadowBlur=r*.2;for(let layer=0;layer<3;layer++){for(let i=0;i<150;i++){const a=i/150*Math.PI*2,hx=16*Math.sin(a)**3,hy=13*Math.cos(a)-5*Math.cos(2*a)-2*Math.cos(3*a)-Math.cos(4*a),j=Math.sin(i*12.37+layer*8.1)*r*.022,px=hx*r/18+j,py=-hy*r/18+j*.4;ctx.globalAlpha=.42+((i+layer)%7)/12;ctx.fillStyle=(i+layer)%5?cfg.accent:'#fffbd7';ctx.beginPath();ctx.arc(px,py,Math.max(.7,r*(.008+(i%4)*.003)),0,7);ctx.fill()}}ctx.globalAlpha=1;ctx.fillStyle='#fffbdc';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`700 ${Math.max(12,r*.2)}px system-ui`;ctx.fillText(label,0,0);ctx.restore()}
  function drawSpiral(){ctx.save();ctx.globalCompositeOperation='screen';for(let arm=0;arm<3;arm++){ctx.beginPath();for(let i=0;i<80;i++){const rr=.18+i*.026,a=arm*Math.PI*2/3+rr*3.2,p=project({x:Math.cos(a)*rr,y:0,z:Math.sin(a)*rr});if(i===0)ctx.moveTo(p.x,p.y);else ctx.lineTo(p.x,p.y)}ctx.strokeStyle=arm===1?'rgba(255,255,220,.82)':'rgba(255,224,70,.72)';ctx.lineWidth=Math.max(1.4,Math.min(w,h)*.005);ctx.shadowColor=cfg.glow;ctx.shadowBlur=18;ctx.stroke()}ctx.restore()}
  ...
  async function startMusic(){try{await audio.play();playing=true;music.textContent='Pausar';music.setAttribute('aria-pressed','true')}catch(error){playing=false;music.textContent='Reproducir';music.setAttribute('aria-pressed','false');console.error('No se pudo reproducir Camilo Septimo - orbita.mp3:',error)}}
  function stopMusic(){audio.pause();playing=false;music.textContent='Musica';music.setAttribute('aria-pressed','false')}
  audio.addEventListener('error',()=>{playing=false;music.textContent='Archivo no encontrado';music.setAttribute('aria-pressed','false')});
